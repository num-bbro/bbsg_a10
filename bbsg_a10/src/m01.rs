...
#[template(path = "m01.html")]
pub struct WebTemp {
    name: String,
    rows: Vec<WebTempRow>,
}

pub async fn m01() -> WebTemp {
    let sbrw = get_sbrw00();
    let pea = get_pea00();
    ...
    */
    for (_i, sr) in sbrw.iter().enumerate() {
        ...
        */
        let unba = sr.v[VarType::UnbalPowLossKw as usize].v;
        let ovld = sr.v[VarType::CntTrSatLoss as usize].v;
        let povl = sr.v[VarType::CntTrUnbalLoss as usize].v;
        let smal = sr.v[VarType::SmallSell as usize].v;
