...
pub async fn tr01(para: Query<Param>) -> WebTemp {
    ...
    // ==== decode nor data
    let Ok((norv, _)): Result<(Vec<PeaAssVar>, usize), _> =
        bincode::decode_from_slice(&buf[..], bincode::config::standard())
    else {
    ...
    // ==== decode nor data
    let Ok((evs, _)): Result<(Vec<PeaAssVar>, usize), _> =
        bincode::decode_from_slice(&buf[..], bincode::config::standard())
    else {
    ...
    /*
    assv.sort_by(|b, a| {
        a.v[VarType::NewCarRegVp01.tousz()]
            .v
            .partial_cmp(&b.v[VarType::NewCarRegVp01.tousz()].v)
            .unwrap()
    });
    norv.sort_by(|b, a| {
        a.v[VarType::NewCarRegVp01.tousz()]
