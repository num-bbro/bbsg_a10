...
pub fn sum_stage_e(
    ...
    let prv_brn_ass = sum_stage_prv_brn(coreno, ass, &mut prv_sub_ass, &mut brn_ass)?;

    let mut brn_ass1 = sum_stage_brn1(coreno, ass, &mut brn_ass)?;

    let prv_brn_ass1 = sum_stage_prv_brn(coreno, ass, &mut prv_sub_ass, &mut brn_ass1)?;
    ...
    let prv_brn_ass2 = sum_stage_prv_brn(coreno, ass, &mut prv_sub_ass, &mut brn_ass2)?;
            ...
            for subasc in subass.chunks_mut(ssz) {
                ...
                s.spawn(move || {
                    for (vas, _vis) in subasc.iter_mut() {
                        ...
                        if let Some(sx) = sbex1.get(&vas.sbid) && sub_bess.contains(&vas.sbid) {
                            vas.n1d = sx.n1d_f;
...
pub fn sum_stage_prv_brn(
    ...
    ass: &Assumption,
    //_tras_raw: &mut [PeaAssVar],
    //prv_sub_ass: &mut [(PeaAssVar,Vec<usize>)],
    //branch_ass: &mut [(PeaAssVar,Vec<usize>)],
    ...
    for ((vas,_),(vas0,_)) in prv_brn_ass.iter_mut().zip(prv_sub_ass.iter()) {
        if vas.pvid!=vas0.pvid { println!("??????===========  {} {}", vas.pvid, vas0.pvid); };
        vas.v[VarType::BessMWh as usize].v = vas0.v[VarType::BessMWh as usize].v;
        println!("!!! BESS {} = {}", vas.pvid, vas0.v[VarType::BessMWh as usize].v);
        vas.v[VarType::ChgStnCap as usize].v = vas.v[VarType::ChgStnCapTr as usize].v;
        vas.v[VarType::ChgStnSell as usize].v = vas.v[VarType::ChgStnSellTr as usize].v;
        vas.v[VarType::MvPowSatTr as usize].v =
            vas.v[VarType::MaxPosPowSub as usize].v / z2o(vas.v[VarType::SubPowCap as usize].v);
...
) -> Result<PeaAssWithMemb, Box<dyn Error>> {
