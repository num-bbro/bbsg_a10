...
pub fn stage_02_b(coreno: usize, ac: &Archi, assrw1: Vec<PeaAssVar>) -> Result<(), Box<dyn Error>> {
                ...
                for subasc in subass.chunks_mut(ssz) {
                    ...
                    s.spawn(move || {
                        for (vas, _vis) in subasc.iter_mut() {
                            ...
                            vas.sum_yr(VarType::FirMvReThb, &ac);
                            // CHECK MEMBERS
    ...
    */
    ...
    let mut svg_v: Vec<_> = subass
        .iter()
        .enumerate()
        .map(|(i, s)| (s.0.v[VarType::SubSolarEnergy as usize].v, i))
        .collect();
    svg_v.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap());
    for (r, (_, i)) in svg_v.iter().enumerate() {
        if r < svg_no {
    ...
    for pvas in prvass.iter_mut() {
        ...
        pvas.sum_yr(VarType::FirTpaThb, ac);
    }
...
pub fn ass_calc(sbas: &mut PeaAssVar, ac: &Archi) -> Result<(), Box<dyn Error>> {
    // ==========  LOSS CALCULATION
    ...
    sbas.sum_yr(VarType::FirUnbSave, ac);
    sbas.sum_yr(VarType::FirTrSatSave, ac);
    sbas.sum_yr(VarType::FirTrPhsSatSave, ac);
    sbas.sum_yr(VarType::FirNonTechLoss, ac);
    ...
    sbas.sum_yr(VarType::CstReinvest, ac);
